...
const Delivery = () => {
  ...
  const columns = ["ID", "Delivery Note No","Date/time", "Type", "Customer", "Customer Office", "Purchase Order No",
    "Store","Transaction Type", "Credit pay", "Due", "Purchase Order img/pdf", "Print"];
  ...
  const fetchSalesHistory = async () => {
    try {
      ...
      const filteredInvoices = invoices.filter(invoice => invoice.status === "delivery" &&  invoice.draft==="false");
      ...
      const formattedData = filteredInvoices.map((invoice, index) => {
        ...
        const currentYear = invoiceDate.getFullYear().toString().slice(-2);

        return [
          invoice.invoiceId,
          'DN-'+invoice.invoiceNo+'-'+invoice.deliveryTime+'-'+currentYear,
          ...
          <div>
            <Link to={`/createDelivery/${invoice.store}/${invoice.invoiceNo}`}><button className="btn btn-primary" style={{fontSize: "12px"}}>Print</button></Link>
          </div>,
        ];
      });
      ...
      <div className="scrolling-container">
        <div className="new-sales-container">
          ...
          <div className="payment-form-button d-grid d-md-flex me-md-2 justify-content-end ">
            <Link to={`/createDelivery/${invoice.store}/${invoice.invoiceNo}`}> <button className="btn btn-warning" style={{fontSize: "12px"}}>Create Delivery Note</button></Link>
          </div>
          <Table
            data={data}
            columns={columns}
