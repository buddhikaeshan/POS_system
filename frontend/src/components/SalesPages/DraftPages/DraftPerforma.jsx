...
const DraftCredit = () => {
  ...
  const columns = ["ID", "Performa Invoice No", "Date/time", "Type", "Customer", 'Customer Office', "Purchase Order No", "store",
     "Transaction Type"  , "Creadit Pay", "Due", "Purchase Order img/pdf","Draft"];
  ...
  const fetchSalesHistory = async () => {
    try {
      ...
      const filteredInvoices = invoices.filter(invoice => invoice.performa === "true" &&  invoice.draft === "true");
      ...
      const formattedData = filteredInvoices.map((invoice, index) => {
        ...
        return [
          ...
          // <div>
          //   <Link to={`/createPerforma/${invoice.store}/${invoice.invoiceNo}`}><button className="btn btn-primary">Print</button></Link>
          // </div>,
          <div>
  ...
  const title = 'Draft_Credit';
  const invoice = 'Draft_Credit.pdf';
      ...
      <div className="scrolling-container">
        <div className="new-sales-container">
          ...
          {/* <div className="payment-form-button d-grid d-md-flex me-md-2 justify-content-end ">
            <Link to={`/createPerforma/${invoice.store}/${invoice.invoiceNo}`}> <button className="btn btn-warning">Create Performa Invoice</button></Link>
          </div> */}
          <Table
            data={data}
            columns={columns}
